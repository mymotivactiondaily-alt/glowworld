...
export const MascotCompanion: React.FC<MascotCompanionProps> = ({ 
  ...
  useEffect(() => {
    if (!isVisible || state !== 'idle' || isMinimized || dismissed) {
      setShowBubble(false);
      return;
    }
  ...
  }, [isVisible, state, isMinimized, dismissed]);
  ...
  return (
    ...
    >
      <AnimatePresence>
        {showBubble && !isMinimized && state === 'idle' && (
          <motion.div 
          ...
          >
             ...
             >
                ...
                <p className="text-white text-sm font-bold leading-relaxed mb-4 mt-2">
      ...
      >
        ...
        <motion.div
          ...
          className={`relative z-0 ${isMinimized ? 'w-14 h-14 opacity-85' : 'w-28 h-28 md:w-36 md:h-36'}`}
        >
